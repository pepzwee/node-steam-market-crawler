...
function MarketItem($, loadEndpoints) {
    ...
    this.url = $( Elem.market_hash_name ).last().attr('href');

    this.market_hash_name = decodeURIComponent(this.url).split('/')[6] || null;
    this.market_name = $( Elem.market_name ).text() || this.market_hash_name;
